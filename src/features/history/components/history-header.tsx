...
export function HistoryHeader({ viewMode, onToggleView }: HistoryHeaderProps) {
  ...
  return (
    <header className="flex items-center justify-between gap-3">
      <h1 className="text-2xl font-bold text-foreground">{HISTORY_COPY.title}</h1>
      ...
      <div className="flex items-center gap-2">
        <button
          ...
          className="flex size-10 items-center justify-center rounded-2xl border border-foreground/10 bg-card text-foreground/60 transition-colors hover:bg-mint/40 hover:text-primary"
        >
          <ViewIcon className="size-5" strokeWidth={2} aria-hidden="true" />
        </button>
        ...
        <button
          ...
          className="flex size-10 items-center justify-center rounded-2xl border border-foreground/10 bg-card text-foreground/30"
        >
